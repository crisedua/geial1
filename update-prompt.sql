-- Update the existing prompt with the correct template including testimonial and GEIAL description
UPDATE prompts 
SET 
  system_prompt = 'Rol
...
SELECT 
  ...
  LENGTH(user_prompt_template) as user_prompt_length,
  CASE WHEN user_prompt_template LIKE '%{{testimonial}}%' THEN 'YES' ELSE 'NO' END as has_testimonial,
  CASE WHEN user_prompt_template LIKE '%{{GEIAL}}%' THEN 'YES' ELSE 'NO' END as has_geial,
  CASE WHEN user_prompt_template LIKE '%{{DESC_GEIAL}}%' THEN 'YES' ELSE 'NO' END as has_desc_geial,
  updated_at
FROM prompts 
WHERE name = 'comunicado-generation';
